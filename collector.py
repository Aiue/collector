...
class RemoteFile:
    ...
    def get(self):
        ...
        if (time.time() - self.requests['last']) < config.min_request_interval:
            sleep_time = config.min_request_interval - (time.time() - self.requests['last'])
            logger.debug('Request limit reached, sleeping for %f seconds.', sleep_time)
...
>>>>>>> e910feced7d7e2dc0651190660a8db459a497b45
        ...
        try:
            ...
            logger.debug('Downloaded %d bytes in %f seconds.', download_size, time.time() - time_start)
...
def main():
    ...
    while True:
        ...
        if not domain:
            ...
            monitor.state.state('idle')
            monitor.UpdateStatus(current_domain='N/A', current_archive='N/A')
